...
def print_command_dict (command_dict: dict, indent_break_max = 0):
    ...
    def __print_recur (subdict: dict, indent = 0, key_len_max = 0, indent_break_max = 0):
        ...
        for key, val in subdict.items():
            ...
            if type(val) == dict:
                ...
                if len(subitems) == 1 and type(subitems[0]) in __types_single:
                    ...
                    print(__ichar*indent + print_prefix + print_suffix)  # Print
                ...
                # Multiple subitems: expanded dict writing (recursive call) - no formatting adjustment with spaces
                    ...
                    print(__ichar*indent + f'{__serialize(key)} = {{')
                    __print_recur(val, indent=indent+__iincr, key_len_max=key_len_max_)
                    print(__ichar*indent + '}')
            ...
            # Unpack lists to assign the key to each subdictionary
                ...
                if all((type(v) in __types_single  for v in val)):
                    print_prefix = f'{__serialize(key)} '
                    print(__ichar*indent + print_prefix + f'= {val[0]}')
                    for item in val[1:]:
                        print(__ichar*indent + ' '*len(print_prefix) + f'= {val[0]}')

                # List contains dictionary elements
                else:
                    for item in val:
                        __print_recur({key: item}, indent=indent)  # Distribute print_recur across all subdicts contained in the list
            ...
            # End node (leaf)
                ...
                if key == '__inline__':
                    print(__ichar*indent + __serialize(val))
                ...
                # Regular key-pair formatted as "key = value"
                    ...
                    if key_len_max > len(print_prefix): print_prefix += ' ' * (key_len_max - len(print_prefix))
                    print(__ichar*indent + print_prefix + print_suffix )
            
            if indent < indent_break_max:
                print()
    
    return __print_recur(command_dict, indent=0, key_len_max=0, indent_break_max=indent_break_max)
